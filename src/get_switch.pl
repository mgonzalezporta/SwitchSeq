...
##############################################################
# TO DO
# ask Pol for feedback on the code structure
# parse imput - how many pc genes + how many are not
# what if two major transcripts are equally abundant within a sample?
# expand to all biotypes of genes
##############################################################
...
# while( my $row = <INPUT>)  {
# 	#if ($has_header eq "yes") { next if $.==1 };
# 	next if $.==1;
...
# 			print OUT "$tId_cond1 $isPrincipal_cond1 $tBiotype_cond1 $tExp_count_cond1 $gExp_count_cond1 $breadth_cond1 ";
# 			print OUT "$tId_cond2 $isPrincipal_cond2 $tBiotype_cond2 $tExp_count_cond2 $gExp_count_cond2 $breadth_cond2 ";
# 			print OUT "$pIdentity $pdbEntry $pdbId $pdbCoverage $rank\n";
# 		}		
# 	}
# }
# close (OUT);
...
# generate html
system("perl ./src/generate_html.pl --out_dir $out_dir -p $plot");
